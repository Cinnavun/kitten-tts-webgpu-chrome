...
const RE_DATE = /(?<![\w/-])(?:(\d{4})[-/.](\d{1,2})[-/.](\d{1,2})|(\d{1,2})[-/.](\d{1,2})[-/.](\d{4}))(?![\w/-])(?!\.\d)/g;
...
function ordinalSuffix(n) {
  ...
  if (!lastOrd) {
    if (last.endsWith("y")) lastOrd = last.slice(0, -1) + "ieth";
...
function validDay(m, d, y) {
  ...
  const max = (m === 2 && !leap) ? 28 : DAYS_IN_MONTH[m - 1];
  return d <= max;
}

export function expandDates(text) {
  return text.replace(RE_DATE, (m, isoY, isoM, isoD, usM, usD, usY) => {
    let y, mo, d;
    if (isoY) { y = +isoY; mo = +isoM; d = +isoD; }
    else {
      y = +usY; mo = +usM; d = +usD;
      if (mo > 12 && d >= 1 && d <= 12) [mo, d] = [d, mo]; // DD-MM-YYYY rescue
    }
    if (y < 1000 || y > 2199 || !validDay(mo, d, y)) return m;
    return `${MONTHS[mo - 1]} ${ordinalSuffix(d)}, ${yearToWords(y)}`;
  });
}
...
export class TextPreprocessor {
  ...
  process(text) {
    ...
    if (cfg.expand_dates) text = expandDates(text);
